...
public class Units {
	...
	{ 
		...
		System.out.println("Es werden " + width*2 + " Attacken ausgeführt."); // die erste und zweite Reihe können kämpfen
		...
		Random dice = new Random();
			for (int i = 0; i < width*2; i++) 	// so oft Würfel wie Modelle im ersten und zweiten Glied
				{
...
//				System.out.println(roll);		// Würfelergebnisse anzeigen
	...
	{ 
		...
		if(et>strength+1)			// ist der Widerstand um mindestens 2 größer als die Stärke, dann auf 6
			dicetowound = 6;
		else if(et>strength)		// ist der Widerstand nicht mindestens um 2 größer, aber größer, dann auf 5
			dicetowound = 5;
		else if (strength==et)		// ist der Widerstand gleich der Stärke, dann auf 4
			dicetowound = 4;
		else if (strength==et+1)	// ist die Stärke genau um 1 größer als der Widerstand, dann auf 3
			dicetowound = 3;
		else						// sonst auf 2
			dicetowound = 2;
		
		System.out.println("Stärke " + strength + " gegen Widerstand " + et + " bedeutet, dass mindestens eine " + dicetowound + " zum Verwunden benötigt wird.");
...
//			System.out.println(roll);
	...
	public int armour(int wounds, int ea) //wounds = Anzahl der Verwundungen, ea = Rüstungswurf des Gegners
	{ 
		if (wounds==0)
			return 0;
		else
		...
		if (armourmodifier<1)
			...
			System.out.println("Stärke " + strength + " bedeutet, dass der Rüstungswurf des Gegners von " + ea + " auf " + armourlast + " modifiziert wird.");
			}
...
//				System.out.println(roll);
				...
				else System.out.println("Kein Lebenspunktverlust, da der Rüstungswurf von " + armourlast + " mit " + roll + " geschafft wurde.");
				}	
			if(armourmiss==0)
	...
	public int ward(int armourmiss, int ew) // armourmiss = Anzahl der abzulegenden Rettungswürfe, ew = Rettungswurf des Gegners
	{ 
		if (armourmiss==0)
			return 0;
		else
